...
public static partial class HotkeysManager
{
    ...
    public static bool DoesTheSameHotkeyExist(GlobalHotkey hotkey, Key key, ModifierKeys modifierKeys)
    {
        return Hotkeys.Where(globalHotkey => globalHotkey != hotkey).Any(globalHotkey => globalHotkey.Key == key && globalHotkey.Modifier == modifierKeys);
    }

    private static void CheckHotkeys()
    {
        foreach (var hotkey in Hotkeys
                     .Where(hotkey => Keyboard.Modifiers == hotkey.Modifier && hotkey.Key != Key.None && Keyboard.IsKeyDown(hotkey.Key))
